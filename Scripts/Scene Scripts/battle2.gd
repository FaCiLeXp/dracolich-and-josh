...
func enemy_turn():	
	...
	if current_enemy_health_3 > 0:
		if is_defending:
			...
			await textbox_closed
	if turns == 5:
		display_text("The %s is charging up a massive attack." % enemy2.name)
		await textbox_closed
...
func check_death():
	...
	if current_enemy_health_3 == 0:
		...
		if minions > 2:
			...
			set_health(enemies[2].get_node("ProgressBar"), current_enemy_health_3, enemy3.health)
		await get_tree().create_timer(0.25).timeout
	emit_signal("check_death_sig")

func enemy_hurt(enemy_current_health, enemy_max_health, damage, enemy_number, hurt_anim):
	enemy_current_health = max(0, enemy_current_health - damage)
	set_health(enemies[enemy_number].get_node("ProgressBar"), enemy_current_health, enemy_max_health)
